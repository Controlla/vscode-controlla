...
export class Dependencies {
  ...
  // private s3urlprefix = 'https://controlla-cli.s3-us-west-2.amazonaws.com/';
...
//     return path.join(this.extensionPath, 'controlla-master', 'controlla', 'cli.py');
//   }

//   public getStandaloneCliLocation(): string {
//     const ext = Dependencies.isWindows() ? '.exe' : '';
//     return path.join(this.extensionPath, 'controlla-cli', 'controlla-cli' + ext);
//   }
...
//             this.logger.debug(`Current controlla-cli version is ${currentVersion}`);

//             this.logger.debug('Checking for updates to controlla-cli...');
//             this.getLatestCliVersion(latestVersion => {
//               if (currentVersion === latestVersion) {
//                 this.logger.debug('controlla-cli is up to date');
//                 callback(true);
//               } else if (latestVersion) {
//                 this.logger.debug(`Found an updated controlla-cli v${latestVersion}`);
//                 callback(false);
//               } else {
//                 this.logger.debug('Unable to find latest controlla-cli version');
//                 callback(false);
//               }
//             });
//           } else {
...
//     let url = 'https://raw.githubusercontent.com/controlla/controlla/master/controlla/__about__.py';
//     this.options.getSetting('settings', 'proxy', (proxy: string) => {
//       this.options.getSetting('settings', 'no_ssl_verify', (noSSLVerify: string) => {
//         let options = { url: url, proxy: '', strictSSL: true };
//         if (proxy) { options['proxy'] = proxy;}
//         if (noSSLVerify === 'true') {options['strictSSL'] = false;}
//         request.get(options, (error: any, response: { statusCode: number; }, body: string) => {
//           let version: string = '';
...
//     this.logger.debug('Downloading controlla-cli...');
//     let url = 'https://github.com/controlla/controlla/archive/master.zip';
//     let zipFile = path.join(this.extensionPath, 'controlla-master.zip');
...
//     this.logger.debug('Downloading controlla-cli standalone...');
//     const url = this.s3BucketUrl() + 'controlla-cli.zip';
//     let zipFile = path.join(this.extensionPath, 'controlla-cli.zip');
//     this.downloadFile(url, zipFile, () => {
...
//     this.logger.debug(`Extracting controlla-cli into "${this.extensionPath}"...`);
//     this.removeCli(() => {
//       this.unzip(zipFile, this.extensionPath, callback);
//       this.logger.debug('Finished extracting controlla-cli.');
//     });
//   }
...
//       this.unzip(zipFile, this.extensionPath, callback);
//       this.logger.debug('Finished extracting controlla-cli standalone.');
//     });
//   }

//   private removeCli(callback: () => void): void {
//     if (fs.existsSync(path.join(this.extensionPath, 'controlla-master'))) {
//       try {
//         rimraf(path.join(this.extensionPath, 'controlla-master'), () => {
//           callback();
//         });
//       } catch (e) {
//         this.logger.warn(e);
...
//     if (fs.existsSync(path.join(this.extensionPath, 'controlla-cli'))) {
//       try {
//         rimraf(path.join(this.extensionPath, 'controlla-cli'), () => {
//           callback();
//         });
//       } catch (e) {
//         this.logger.warn(e);
...
//     } else {
//       let error_msg =
//         'controlla depends on Python. Install it from https://python.org/downloads then restart VS Code';
